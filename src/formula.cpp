...
: _op (op), _unique (NULL)
{
   ...
   _int = interval;
   if (_int != NULL) {
        if (_max_bound < _int->_right)
            _max_bound = _int->_right;
   }
   clc_hash ();
...
Formula::build (const mtl_formula *formula, bool is_not)
{
  ...
  switch (formula->_type)
    ...
    case eGLOBALLY: // G a = False R a -- [!(G a) = True U !a]
      ...
      _max_bound = (_int->_right > _right->_max_bound) ? _int->_right : _right->_max_bound;
      break;
    ...
    case eUNTIL: // a U b -- [!(a U b) = !a R !b]
      ...
      _max_bound = (_int->_right > _right->_max_bound) ? _int->_right : _right->_max_bound;
      _max_bound = (_max_bound > _left->_max_bound) ? _max_bound : _left->_max_bound;
      break;
    ...
    case eAND: // a & b -- [!(a & b) = !a | !b ]
      ...
      _left = Formula (formula->_left, is_not).unique ();
      _right = Formula (formula->_right, is_not).unique ();
      _max_bound = (_left->_max_bound > _right->_max_bound) ? _left->_max_bound : _right->_max_bound;
      break;
...
Formula::clone () const
{
  ...
  if (_int != NULL)
    ...
    new_formula->_max_bound = _max_bound;
